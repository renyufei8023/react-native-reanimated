...
export default function SpringNewConfigWithClampExample() {
  ...
  const newDefaultConfig: WithSpringConfig = {
    damping: 110,
    mass: 4,
    stiffness: 900,
  ...
  const newDefaultConfigLowStyle = useAnimatedStyle(() => {
    return {
      width: withSpring(lowWidth.value / 3, newDefaultConfig),
    };
  });

  const newDefaultConfigMediumStyle = useAnimatedStyle(() => {
    return {
      width: withSpring(mediumWidth.value / 2, newDefaultConfig),
    };
  });

  const newDefaultConfigHighStyle = useAnimatedStyle(() => {
    return {
      width: withSpring(highWidth.value, newDefaultConfig),
    };
  });
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Visualiser
        testedStyle={newDefaultConfigLowStyle}
        description="Old default config, no timing, low value"
      />
      <Visualiser
        testedStyle={newDefaultConfigMediumStyle}
        description="Old default config, no timing, medium value"
      />
      <Visualiser
        testedStyle={newDefaultConfigHighStyle}
